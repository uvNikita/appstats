...
class Counter():
    ...
    def __get_names(self):
        names = []
        for key in self.db.keys("%s*.%s.%s.last_val.%s" % (self.prefix, self.interval, self.part, self.fields[0])):
            names += ['.'.join(key.split('.')[1:-4])] # Example key format: statistics.name.3600.60.last_val.REQUESTS
        return names
